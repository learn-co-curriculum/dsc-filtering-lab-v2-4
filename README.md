...
cur = conn.cursor() #Used cur vs. cursor for shorthand
```
...
cur.execute("""CREATE TABLE planets
                                            (id INTEGER PRIMARY KEY,
                                                 ...
                                                 mass REAL);
                    """)
#Note: if you run this code from the solution branch, it will error because the planets table already exists in planets.db
#If you really want to play around you could first try executing the sql query """DROP TABLE IF Exists planets ;"""
```




    <sqlite3.Cursor at 0x2396f1aef80>
...
cur.execute("""INSERT INTO planets (name, color, num_of_moons, mass, rings) VALUES ("Mercury", "gray", 0, 0.55, 0),
...
cur.execute("""UPDATE planets SET num_of_moons = 68 WHERE name = "Jupiter";""")
```
...
cur.execute("""DELETE from planets WHERE name = "Pluto";""")
```
...
Finally, to save our changes so that other users will see these updates, we must commit them via our connection.
